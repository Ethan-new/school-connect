  ...
  useEffect(() => {
    ...
    async function open() {
      ...
      try {
        ...
        const msgsRes = await getMessagesAction(convId);
        if (msgsRes.success) {
    ...
    open();
  }, [isOpen, guardianId, studentId, schoolId, existingConversationId]);
      ...
      >
        <div className="flex shrink-0 items-center justify-between border-b border-zinc-200 px-4 py-3">
          <div>
            ...
            <p className="text-sm text-zinc-500">Parent of {studentName}</p>
          </div>
          <button
            type="button"
            onClick={onClose}
